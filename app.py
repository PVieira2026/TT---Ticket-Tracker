"""TT Tracker — Streamlit webapp. Reads from Google Sheets via Service Account or public CSV."""
import os, json
from datetime import datetime
import streamlit as st
import pandas as pd

st.set_page_config(page_title="TT Tracker", page_icon="🎟️",
...
.tt-badge{background:rgba(255,255,255,.15);border-radius:20px;padding:6px 14px;color:#fff;font-size:.8rem;font-weight:600;}
...
.stSelectbox>div>div{background:#1A1A24!important;border:1px solid #2D2D3D!important;border-radius:10px!important;color:#F1F1F3!important;}
...
.ec-noimg{width:100%;height:175px;background:linear-gradient(135deg,#1E1B3A,#2D2D3D);display:flex;align-items:center;justify-content:center;font-size:3rem;}
...
.t-bl{background:rgba(99,102,241,.15);color:#A5B4FC;}
.t-bo{background:rgba(245,158,11,.15);color:#FDE68A;}
...
.pr{margin-top:auto;padding-top:12px;border-top:1px solid #2D2D3D;display:flex;align-items:center;justify-content:space-between;}
.pr-ok{font-size:1.1rem;font-weight:800;color:#34D399;}
.pr-na{font-size:.85rem;color:#555;font-style:italic;}
.ebtn{background:#7C3AED;color:#fff!important;border-radius:8px;padding:5px 13px;font-size:.77rem;font-weight:600;text-decoration:none!important;}
.tr{display:flex;justify-content:space-between;align-items:center;padding:8px 0;border-bottom:1px solid #2D2D3D;}
.tr-s{color:#D1D5DB;font-size:.88rem;}
.tr-p{font-weight:700;color:#34D399;font-size:.95rem;}
.tr-sold{color:#EF4444;font-size:.75rem;}
.tr-n{color:#888;font-size:.75rem;font-style:italic;}
.src-badge{font-size:.65rem;padding:2px 7px;border-radius:10px;background:rgba(52,211,153,.1);color:#34D399;margin-left:6px;}
.stTabs [data-baseweb="tab-list"]{gap:8px;background:transparent;}
.stTabs [data-baseweb="tab"]{background:#1A1A24;border:1px solid #2D2D3D;border-radius:10px;color:#888;font-weight:600;padding:8px 20px;}
.stTabs [aria-selected="true"]{background:#7C3AED!important;border-color:#7C3AED!important;color:#fff!important;}
...
def load_data():
    ...
    # Method A: Service Account (works with private sheet)
    if SA_JSON:
        try:
            import gspread
            ...
            data  = ws.get_all_records(expected_headers=COLS)
            ...
            st.warning(f"gspread read failed: {e} — trying public CSV...")

    # Method B: Public CSV (sheet must be public)
    try:
...
def pcls(p):
    ...
    if "blueticket" in p: return "t-bl"
    if "bol" in p: return "t-bo"
    return "t-ei"
    ...
    if not rendered and td:
        for line in td.splitlines():
            ...
            if len(pts)>=2:
                ...
                rendered=True
    if not rendered:
        st.markdown('<p style="color:#555;font-size:.85rem;padding:8px 0">Sem precos disponiveis.</p>',unsafe_allow_html=True)
...
def card(row):
    ...
    src_html=f'<span class="src-badge">{src}</span>' if src else ""
    ih=f'<img class="ec-img" src="{img}">' if img else '<div class="ec-noimg">🎵</div>'
    prh=(f'<span class="pr-ok">{mn:.0f}\u20ac \u2013 {mx:.0f}\u20ac{src_html}</span>'
         if mn>0 or mx>0 else '<span class="pr-na">Em breve</span>')
    lnk=f'<a href="{url}" target="_blank" class="ebtn">Ver \u2192</a>' if url else ""
    st.markdown(f'<div class="ec">{ih}<div class="ec-body"><div class="ec-name">{name}</div><div class="ec-meta"><span class="tag t-dt">📅 {ds}</span><span class="tag {pcls(plat)}">{plat}</span><span class="tag {ccls(cat)}">{cat}</span></div><div class="pr">{prh}{lnk}</div></div></div>',unsafe_allow_html=True)
    with st.expander("🎫 Precos detalhados"):
        render_tickets(row["tickets_json"],row["tickets_detail"])


def grid(df):
    if df.empty:
        st.markdown('<div class="no-res"><div style="font-size:3rem">🔍</div><p>Sem resultados.</p></div>',unsafe_allow_html=True); return
...
def main():
    st.markdown('<div class="tt-hdr"><div><h1>🎟️ TT Tracker</h1><p>Concertos &amp; Festivais em Portugal \u2014 precos em tempo real</p></div><span class="tt-badge">\U0001f1f5\U0001f1f9 Portugal</span></div>',unsafe_allow_html=True)

    with st.spinner("A carregar eventos..."):
        df=load_data()

    if df.empty:
        st.warning("Configure SPREADSHEET_ID + GOOGLE_SERVICE_ACCOUNT_JSON nos Streamlit Secrets.")
        st.code('SPREADSHEET_ID = "id-da-sheet"\nSHEET_GID = "0"\nGOOGLE_SERVICE_ACCOUNT_JSON = \'{"type":"service_account",...}\'', language="toml")
        st.stop()
    ...
    if not last.empty:
        try:
            ...
            st.markdown(f'<div class="ts">🕐 Ultima actualizacao: {ts} UTC</div>',unsafe_allow_html=True)
        except: pass

    c1,c2,c3,c4=st.columns([3,1.5,1.5,1])
    with c1: srch=st.text_input("","",placeholder="🔍  Pesquisar artista, festival...",label_visibility="collapsed")
    with c2:
        pls=["Todas as plataformas"]+sorted(df["platform"].dropna().unique().tolist())
        pf=st.selectbox("",pls,label_visibility="collapsed")
    with c3:
        pf2=st.selectbox("",["Qualquer preco","Ate 30\u20ac","Ate 60\u20ac","Ate 100\u20ac","Ate 150\u20ac","Mais de 150\u20ac"],label_visibility="collapsed")
    with c4:
        if st.button("🔄",help="Refresh",use_container_width=True): st.cache_data.clear(); st.rerun()

    f=df.copy()
    if srch.strip(): f=f[f["name"].str.contains(srch.strip(),case=False,na=False)]
    if pf!="Todas as plataformas": f=f[f["platform"]==pf]
    if pf2!="Qualquer preco":
        def ok(r):
            mn2,mx2=pp(r["price_min"]),pp(r["price_max"])
            if mn2==0 and mx2==0: return True
            p2=mn2 if mn2>0 else mx2
            if   pf2=="Ate 30\u20ac":    return p2<=30
            elif pf2=="Ate 60\u20ac":    return p2<=60
            elif pf2=="Ate 100\u20ac":   return p2<=100
            elif pf2=="Ate 150\u20ac":   return p2<=150
    ...
    for col2,n,l in[(s1,tot,"Total"),(s2,con,"Concertos"),(s3,fst,"Festivais"),(s4,oth,"Outros"),(s5,wpr,"Com precos")]:
        col2.markdown(f'<div class="sp"><div class="n">{n}</div><div class="l">{l}</div></div>',unsafe_allow_html=True)
    st.markdown("<br>",unsafe_allow_html=True)

    t1,t2,t3,t4=st.tabs([f"🎵 Todos ({tot})",f"🎤 Concertos ({con})",f"🎪 Festivais ({fst})",f"🎭 Outros ({oth})"])
    with t1: grid(f)

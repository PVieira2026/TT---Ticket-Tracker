...
def load_data():
    ...
    import io as _io, urllib.request as _ul
    df=None
    if SA_JSON and len(SA_JSON)>50:
        try:
            import gspread as _gs
            _info=json.loads(SA_JSON)
            _SC=["https://www.googleapis.com/auth/spreadsheets"]
            try: _gc=_gs.service_account_from_dict(_info,scopes=_SC)
            except Exception:
                from google.oauth2.service_account import Credentials as _C
                _gc=_gs.authorize(_C.from_service_account_info(_info,scopes=_SC))
            _ws=_gc.open_by_key(SPREADSHEET_ID).worksheet("Eventos")
            _rows=_ws.get_all_records()
            if _rows: df=pd.DataFrame(_rows).fillna("").astype(str)
        except Exception: df=None
    if df is None:
        _urls=[
            f"https://docs.google.com/spreadsheets/d/{SPREADSHEET_ID}/export?format=csv&gid={GID}",
            f"https://docs.google.com/spreadsheets/d/{SPREADSHEET_ID}/gviz/tq?tqx=out:csv&sheet=Eventos",
        ]
        for _url in _urls:
            try:
                _req=_ul.Request(_url,headers={"User-Agent":"Mozilla/5.0 (X11; Linux x86_64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/120.0.0.0 Safari/537.36"})
                with _ul.urlopen(_req,timeout=30) as _resp: _txt=_resp.read().decode("utf-8")
                if _txt.strip()[:50].lower().startswith(("<!doctype","<html")): continue
                df=pd.read_csv(_io.StringIO(_txt),dtype=str).fillna("")
        ...
        if st.button('✅ Guardar no Sheet',use_container_width=True,key='mm_save',type='primary'):
            if not nm or not dt: st.error('Nome e data são obrigatórios.')
                ...
                try:
                    ...
                    gc=gspread.service_account_from_dict(json.loads(SA_JSON),scopes=SCOPES)

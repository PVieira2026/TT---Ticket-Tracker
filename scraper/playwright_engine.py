...
PRICE_WAIT=["#venueZonesList","ul.list_zone_list","[class*=price]","[class*=preco]","[data-price]",".bt-widget"]
BUY_SELS=["a:has-text('Comprar')","button:has-text('Comprar')","a:has-text('Buy')","a:has-text('Bilhetes')","[class*=buy]","a[href*=compra]"]

# Ticketline-specific: extracts p.zone + price from span.session-generic (base price)
TL_ZONE_JS="""() => {
    const rows=[],seen=new Set();
    const listEl=document.querySelector('#venueZonesList,ul.list_zone_list');
    if(!listEl) return rows;
    listEl.querySelectorAll('li:not(.list_header)').forEach(li=>{
        const zoneEl=li.querySelector('p.zone');
        const priceEl=li.querySelector('p.price');
        const soldEl=li.querySelector('p.soldout_tip');
        if(!zoneEl||!priceEl) return;
        const sector=zoneEl.textContent.trim();
        const soldOut=soldEl?/sem disponibilidade|esgotado|sold.?out/i.test(soldEl.textContent):false;
        let price=0;
        const spanEl=priceEl.querySelector('span.session-generic,span[class*=session]');
        if(spanEl){const m=spanEl.textContent.match(/\\((\\d+(?:[,.]\\d+)?)\\s*\u20ac?\\)/);if(m)price=parseFloat(m[1].replace(',','.'));}
        if(!price){const rawText=Array.from(priceEl.childNodes).filter(n=>n.nodeType===3).map(n=>n.textContent.trim()).join('');const m=rawText.match(/(\\d+(?:[,.]\\d+)?)/);if(m)price=parseFloat(m[1].replace(',','.'));}
        if(!sector||!price||price<1||price>2500) return;
        const key=sector+'|'+price; if(seen.has(key)) return; seen.add(key);
        rows.push({sector,price,note:'',sold_out:soldOut});
    });
    return rows;
}"""
...
GENERIC_JS="""() => {
    ...
    while((node=walker.nextNode())){
        ...
        if(el){const anc=el.closest('[class*="sector"],[class*="categoria"],[class*="zona"],[class*="tarif"],[class*="row"],[class*="item"]');if(anc)label=anc.innerText.split('\\n')[0].trim().substring(0,60);}
        const key=(label||'Geral')+('|'+price); if(seen.has(key)) continue; seen.add(key);
...
class BrowserSession:
    ...
    async def __aenter__(self):
        self._pw,self._browser,self._ctx=await _launch(); return self
    ...
    async def cart(self,url):
        page=await self._ctx.new_page()
        try:
            await page.goto(url,wait_until="domcontentloaded",timeout=self.timeout); await page.wait_for_timeout(2000)
            for sel in BUY_SELS:
                try:
            ...
            if _is_tl(url):
                try: await page.wait_for_selector("#venueZonesList",timeout=6000)
                except: pass

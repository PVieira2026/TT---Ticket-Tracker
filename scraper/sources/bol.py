...
def scrape(sheet_state=None):
    ...
    links,seen=[],set(); re_l=re.compile(r'href="(/Comprar/Bilhetes/[^"?#]+)"',re.I)
    for url in ["https://www.bol.pt/Comprar/Bilhetes/index?categoria=Concertos","https://www.bol.pt/Comprar/Bilhetes/index?categoria=Festivais","https://www.bol.pt/Comprar/Bilhetes/index"]:
        try:
        ...
        try:
            ...
            if not rows:
                try:
                    ...
                    ct=batch_cart([url])
                    if ct.get(url): rows=ct[url]; src="cart"
                except: pass
            prices=[r["price"] for r in rows]; img=re.search(r'property="og:image"[^>]*content="([^"]+)"',html,re.I)
            results.append({"id":f"bol-{slug}","name":name,"date":ed or "","platform":PLATFORM,"category":detect_category(name,url),"price_min":str(min(prices)) if prices else "","price_max":str(max(prices)) if prices else "","url":url,"image_url":img.group(1) if img else "","tickets_json":build_tickets_json(rows),"tickets_detail":build_tickets_detail(rows),"updated_at":datetime.utcnow().isoformat(),"scraper_status":"ok" if prices else "ok_no_prices","price_source":src})

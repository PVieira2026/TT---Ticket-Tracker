...
PRMS={"per_page":100,"_fields":"title,link,date,content","orderby":"modified","order":"desc"}
...
def scrape(sheet_state=None):
    ...
    for post in posts:
        ...
        html=post.get("content",{}).get("rendered",""); text=strip_html(html)
        ...
        results.append({"id":f"ein-{slug}","name":name,"date":event_date,"platform":PLATFORM,
            "category":detect_category(name,url),"price_min":str(min(prices)) if prices else "",
            "price_max":str(max(prices)) if prices else "","url":url,
            "image_url":img_m.group(1) if img_m else "","tickets_json":build_tickets_json(rows),
            "tickets_detail":build_tickets_detail(rows),"updated_at":datetime.utcnow().isoformat(),

"""Google Sheets — batch upsert (avoids 429) + read via service account."""
...
def read_sheet(spreadsheet_id: str, sheet_name: str = "Eventos") -> "pd.DataFrame":
    """Read all rows from sheet. Used by SheetState for skip logic."""
    import pandas as pd
    try:
        gc   = _client()
        ...
        log.warning(f"read_sheet failed: {e}")
        import pandas as pd
...
def upsert_events(events: List[Dict], spreadsheet_id: str, sheet_name: str = "Eventos") -> int:
    """
    Batch upsert — reads all rows once, writes everything in minimal API calls.
    Skipped events ('price_source'=='skipped') are NOT written if sheet already has them with prices.
    """
    gc       = _client()
    ws       = gc.open_by_key(spreadsheet_id).worksheet(sheet_name)
    ...
    new_rows = []
    for ev in events:
        # Don't overwrite existing rows that have prices with "skipped" empty rows
        if ev.get("price_source") == "skipped" and ev["id"] in id_map:
            existing_row = matrix[id_map[ev["id"]]]
            price_idx    = COLUMNS.index("price_min")
            if existing_row[price_idx]:
                continue  # keep existing row with prices intact

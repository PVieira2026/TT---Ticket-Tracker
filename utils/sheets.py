...
def upsert_events(events,spreadsheet_id,sheet_name="Eventos"):
    ...
    if not existing or existing[0]!=COLUMNS: ws.clear(); ws.append_row(COLUMNS); existing=[COLUMNS]
    id_col=COLUMNS.index("id"); id_map={row[id_col]:i for i,row in enumerate(existing) if i>0}
    matrix=[list(r)+[""]*(max(0,len(COLUMNS)-len(r))) for r in existing]; new_rows=[]
    for ev in events:
        if ev.get("price_source")=="skipped" and ev["id"] in id_map:
            if matrix[id_map[ev["id"]]][COLUMNS.index("price_min")]: continue
        row=[str(ev.get(c,"") or "") for c in COLUMNS]
        if ev["id"] in id_map: matrix[id_map[ev["id"]]]=row
    ...
    log.info(f"Upserted {len(events)} events."); return len(events)
